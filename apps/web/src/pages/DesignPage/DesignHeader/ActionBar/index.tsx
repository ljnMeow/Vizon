...
export function ActionBar() {
  ...
  const labels = useMemo(
    () => appMessages[locale].designPage.actionBar,
    [locale],
  );
  ...
  const onExportScreenshot = () => {
    ...
    try {
      ...
      const safeBase =
        sanitizeExportFileBaseName(rawBase) ||
        labels.exportFileDefaultSceneName;
      ...
      void message.error(
        `${labels.exportFailedPrefix}${raw || labels.exportUnknownError}`,
      );
    }
  };
  ...
  const onExportBundle = async () => {
    if (!editor) return;
    try {
      const { blob } = await buildProjectBundle(editor, {
        generator: "apps/web-project-bundle",
      });
  ...
  const onSaveScene = async () => {
    ...
    try {
      ...
      const thumbnailBlob = await fetch(screenshotDataUrl).then((r) =>
        r.blob(),
      );
      const { blob: bundleBlob } = await buildProjectBundle(editor, {
        generator: "apps/web-save-scene",
      });

      if (loadedSceneId) {
        // 已有载入场景：覆盖更新，保持场景 ID 不变
        await updateScene(loadedSceneId, {
          name,
          bundle: bundleBlob,
      ...
      const raw = err instanceof Error ? err.message : String(err);
      void message.error(
        `${labels.saveSceneFailedPrefix}${raw || labels.exportUnknownError}`,
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const onImportDocumentChange: ChangeEventHandler<HTMLInputElement> = async (
    e,
  ) => {
    ...
    try {
      ...
      const before = editor.getVizonDocument({
        generator: "apps/web-actionbar",
      });
      ...
      const raw = err instanceof Error ? err.message : String(err);
      const msg =
        raw === VIZON_IMPORT_ERROR_NO_OBJECT_SNAPSHOT
          ? labels.importNoObjectSnapshot
          : raw || labels.importUnknownError;
      console.error(`${labels.importFailedPrefix}${msg}`);
  ...
  const onImportBundleChange: ChangeEventHandler<HTMLInputElement> = async (
    e,
  ) => {
    const inputEl = e.currentTarget;
    const file = e.target.files?.[0];
    ...
    try {
      ...
      const raw = err instanceof Error ? err.message : String(err);
      console.error(
        `${labels.importFailedPrefix}${raw || labels.importUnknownError}`,
      )
      void message.error(
        `${labels.importFailedPrefix}${raw || labels.importUnknownError}`,
      );
        ...
        groups={[
          {
            ...
            items: [
              ...
              {
                key: "paste",
                label: labels.pasteWithShortcut,
                disabled: !canPaste,
                onClick: () => {
                  setOpen(false);
                  void editor?.pasteFromClipboard();
              ...
              {
                key: "delete",
                label: labels.deleteWithShortcut,
                disabled: !hasSelection,
                onClick: () => {
                  setOpen(false);
                  void editor?.deleteSelected();
              ...
              {
                key: "group",
                label: labels.groupWithShortcut,
                disabled: !canGroup,
                onClick: () => {
                  setOpen(false);
                  void editor?.groupSelected();
              ...
              {
                key: "ungroup",
                label: labels.ungroupWithShortcut,
                disabled: !canUngroup,
                onClick: () => {
                  setOpen(false);
                  void editor?.ungroupSelected();
